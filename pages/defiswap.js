...
export default function Defiswap() {
    ...
    const [flogo,
        getFromLogo] = useState([]);
    const [fname,
        getFromName] = useState(['Swap From']);
    const [faddr,
        getFromAddr] = useState([]);
    const [fdec,
        getFromDec] = useState([]);
    const [tlogo,
        getToLogo] = useState([]);
    const [tname,
        getToName] = useState(['Swap To']);
    const [taddr,
        getToAddr] = useState([]);
    const [tdec,
        getToDec] = useState([]);
    ...
    const config = {
        apiKey: "S0-a7U98kE7vpidDg59El1sFGhcRJj2c",
    ...
    const projectId = '8e16682173935a4257a24ace3a4025cf'
    ...
    async function displayBalance() {
        ...
        const data = await alchemy
            .core
            .getTokenBalances(wallet, tokenContractAddresses)
        data
            .tokenBalances
            .find(item => {
    ...
    async function getPrice() {
        ...
        try {
            await swapOrders
                ...
                .find(item => {
                    document
                        .getElementById("defisource")
                        .innerHTML = item.source;
                })
        } catch (error) {
            document
                .getElementById("defisource")
                .innerHTML = "Pool Not Available";
        }
        var rawvalue = swapOrders.buyAmount / (10 ** tdec)
        var value = rawvalue.toFixed(2);
        document
            .getElementById("to_amount")
            .innerHTML = value
        document
            .getElementById("gas_estimate")
            .innerHTML = swapPriceJSON.estimatedGas;
    }
    ...
    return (
        <Grid.Container gap={1} justify='center'>
            ...
            <Web3Modal projectId={projectId} ethereumClient={ethereumClient}/> {/*<Button rounded color="primary" onPress={connect} css={{boxShadow:'0px 0px 4px #000000'}}>
                <Text
                  css={{ color: "white" }}
                  size={16}
                  weight="bold"
            ...
            <Modal
                ...
                <Modal.Footer>
                    {/*</Modal>Button auto flat color="primary" onClick={connect}>
            Connect Wallet
          </Button>
